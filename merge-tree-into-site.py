...
CSS_BLOCK_START = '''/* ============================================================
   METARA TREE OVERLAY CSS -- GENERATED by merge-tree-into-site.py
   from metara-tree-interactive.html. Do not hand-edit this block --
   edit the standalone source and re-run that script instead.
   ============================================================ */
'''
CSS_BLOCK_END = '\n/* ==== END METARA TREE OVERLAY CSS ==== */\n'

OVERLAY_MARKUP_COMMENT = '''
<!--
  ============================================================================
  METARA TREE OVERLAY MARKUP -- GENERATED, see the matching CSS block in
  metara-master-styles.css (search "METARA TREE OVERLAY CSS"). Source of truth:
...
def build(tree_html, site_html):
    """Returns (site_html, tree_js, tree_css). tree_js must be written to a
    same-origin metara-tree.js file, NOT inlined. tree_css must be appended
    to metara-master-styles.css (wrapped in CSS_BLOCK_START/END markers by
    the caller). See CSP_NOTE below."""
...
def main():
    ...
    p.add_argument('--css',
                    help='Path to metara-master-styles.css to append the tree '
                         'CSS block to. Defaults to metara-master-styles.css '
                         'in the same directory as --out. Any previously '
                         'generated block (marked by CSS_BLOCK_START/END) is '
    ...
    site_result, js_result, css_result = build(tree_html, site_html)

    out_dir = os.path.dirname(os.path.abspath(args.out))
    js_path = os.path.join(out_dir, 'metara-tree.js')
    css_path = args.css or os.path.join(out_dir, 'metara-master-styles.css')

    existing_css = open(css_path, encoding='utf-8').read() if os.path.exists(css_path) else ''
    block_re = re.compile(re.escape(CSS_BLOCK_START) + '.*?' + re.escape(CSS_BLOCK_END), re.S)
    existing_css = block_re.sub('', existing_css)
    new_css = existing_css.rstrip() + '\n\n' + CSS_BLOCK_START + css_result.strip() + CSS_BLOCK_END
    ...
    print("Wrote %s (%d bytes)." % (css_path, len(new_css)))
